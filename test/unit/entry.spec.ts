...
import { buildEntry } from '../../src/domain/entry.js';
import { ValidationError, type ValidationIssue } from '../../src/domain/errors.js';
import { type EntryInput } from '../../src/domain/schemas.js';
...
function baseInput(overrides: Partial<EntryInput> = {}): EntryInput {
  return {
    ...
    debits: [{ account: acct('Cash', AccountType.Asset), amount: Amount.fromMajor(100) }],
    credits: [{ account: acct('Revenue', AccountType.Revenue), amount: Amount.fromMajor(100) }],
    ...overrides,
  };
}
...
function messagesFor(root: string, input: EntryInput, resolve = () => null): string[] {
...
describe('buildEntry', () => {
  ...
  it('rejects a missing amount', () => {
    const msgs = issuesFor({
      ...
      credits: [{ account: acct('Rev', AccountType.Revenue), amount: Amount.fromMajor(10) }],
    } as unknown as EntryInput);
    expect(msgs.some((m) => m.message.includes('amount') || m.path.includes('amount'))).toBe(true);
  });
  ...
  it('rejects when debit and credit do not cancel', () => {
    const msgs = issuesFor(
      baseInput({
        ...
        credits: [{ account: acct('Rev', AccountType.Revenue), amount: Amount.fromMajor(100) }],
      }),
    );
    expect(msgs.some((m) => m.path.length === 0)).toBe(true);
    expect(msgs.some((m) => m.message === 'The credit and debit amounts are not equal')).toBe(true);
  });
  ...
  it('rejects fractional mismatches exactly (100.10 vs 100.20)', () => {
    const msgs = issuesFor(
      baseInput({
        ...
        credits: [
          { account: acct('Rev', AccountType.Revenue), amount: Amount.fromMajor('100.20') },
        ],
      }),
    );
    expect(msgs.some((m) => m.message === 'The credit and debit amounts are not equal')).toBe(true);
  ...
  it('accepts multiple credits summing to the debit total', () => {
    const payload = buildEntry(
      baseInput({
        ...
        credits: [
          { account: acct('Sales', AccountType.Revenue), amount: Amount.fromMajor(45) },
          { account: acct('Tax', AccountType.Liability), amount: Amount.fromMajor(5) },
        ],
      }),
    );
    ...
    const msgs = issuesFor(
      {
        ...
        credits: [{ account: acct('Revenue', AccountType.Revenue), amount: Amount.fromMajor(10) }],
      },
      () => null,
    );
    expect(msgs.some((m) => m.message === 'Account "Ghost" not found')).toBe(true);
  });
  ...
  it('accepts raw number amounts (Zod transform)', () => {
    const payload = buildEntry({
      ...
      credits: [{ account: acct('Rev', AccountType.Revenue), amount: '100.00' }],
    });
    const [debit] = payload.debits;
    if (!debit) throw new Error('no debit');
    const [credit] = payload.credits;
